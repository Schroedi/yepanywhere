...
| Agent command runtime | Start the [ya-agent proposal](../../topics/agent-command-runtime.sketches.md#first-milestone-inspect-the-owning-session) with read-only `self` reporting ownership and model/effort evidence. Use operator-managed global instructions initially; defer automatic advertisement, private input, broader session access, and scheduling integration. |
