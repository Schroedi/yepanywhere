...
| Related work across repositories | The [Issues & PRs MVP plan](../tactical/125-issue-session-associations.md) requires automatic ticket/URL indexing from viewed sessions and a configurable recent-session window, with durable evidence and SQLite migrations. Implementation has not started; wire compatibility review is next. Workstream/branch inference, multi-server aggregation and tracker synchronization are deferred. |
